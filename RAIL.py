...
class train():
    ...
    def booking_tickets(self,num_tickets):
        if num_tickets >self.seats:
            return None
        else:
            pnr_list=[]
            for i in range(num_tickets):
                pnr_list.append(random.randint(7000,9000))
...
if logged_in_account is not None:
    trains=[
        train("34862","secunderabad","chennai",80),
        train("78542","moulali","Banglore",70),
        train("68231","falaknama","Ooty",60),
    ]
...
#abstract user input for booking
while True:
    try:
...
if train is None:
    ...
    pnr_list=train.booking_tickets(num_tickets)
    if pnr_list is None:
